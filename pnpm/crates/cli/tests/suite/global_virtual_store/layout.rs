...
/// Installing with symlink=false and the isolated linker without `PnP` is a
/// config conflict (TS misc.ts). The isolated layout links the virtual
/// store and importer dependencies with symlinks, so disabling them
/// without `PnP` would leave `node_modules` without direct dependencies
/// (pnpm/pnpm#8951).
#[test]
